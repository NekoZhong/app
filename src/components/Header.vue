<template>
  <div class="header">
    ...
    <div class="bulletin-wrapper">
			<img class="icon" v-if="shopIn.discounts2" :src="shopIn.discounts2[0].icon_url" />

			<span class="text" v-if="shopIn.discounts2">{{shopIn.discounts2[0].info}}</span>
			<div class="detail" v-if="shopIn.discounts2" @click="showBulletin">
				{{shopIn.discounts2.length}}个活动
      ...
      <div class="bulletin-detail" v-show="isShow">
          <div class="detail-wrapper">
            ...
            <div class="main-wrapper" :style="detail_bg">
              ...
              <h3 class="name">{{shopIn.name}}</h3>
              <!-- 星级评价 -->
              <div class="score">
                <app-star :score="shopIn.wm_poi_score"></app-star>
                <span>{{shopIn.wm_poi_score}}</span>
              </div>

              <p class="tip">
                {{shopIn.min_price_tip}} <i>|</i> {{shopIn.shipping_fee_tip}} <i>|</i> {{shopIn.delivery_time_tip}}
              </p>

              <p class="time">
                配送时间: {{shopIn.shipping_time}}
              </p>

              <div class="discounts" v-if="shopIn.discounts2">
                <p>
                  <img :src="shopIn.discounts2[0].icon_url" />
                  <span>{{shopIn.discounts2[0].info}}</span>
                </p>
              </div>
            </div>
...
export default {
  ...
  computed:{
    head_pic_url(){
      return "background-image: url(" + this.shopIn.head_pic_url + ");"
    },
    head_bg(){
      return "background-image: url(" + this.shopIn.pic_url + ");"
    },
    detail_bg(){
      return "background-image: url(" + this.shopIn.poi_back_pic_url + ");"
    }
